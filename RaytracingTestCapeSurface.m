...
%7. TO SOLVE: Fresnel Coefficients (amplitude) for AR simulation
%8. Angle change for FEKO (currently supports manual or Royce data)
...
%       way to define it is Radio(frequency,power,phase,location,orientation,pattern)
...
%       angle] where angles are in degrees, and anticlockwise is the positive direction, for example [45 0 0]
%       means to rotate the antenna, with X axis for 45 degrees
%       anti-clockwise
%       For pattern, there are two ways to define a pattern
%       1. By letting patternFromFile=0, and manually specify a gain
...
receiver_pattern=@(theta,phi) 1.5*sin(theta)^2;%pattern for receivers
...
mesh_.xNodeNum = 180;   % Keep the x and y mesh size the same, increase the size for better resolution and especially if you're increasing the frequency
mesh_.yNodeNum = 180;
mesh_.zNodeNum = 1;
...
if patternFromFile==1
    [theta,phi,gain_Lin,AR_Lin] = ReadPattern();
    pattern=[theta,phi,gain_Lin];
    for idx=1:size(power,1)
        transmitter=Radio(frequency,power(idx,:),phase(idx,:),location(idx,:),orientation(idx,:),pattern);%transmitter
...
receiver=Radio(frequency,1,1,Rx.xyz(1,:),[0 0 0],patterngen(receiver_pattern));
...
if losFlag==1%Line of sight
    ...
    for idx=1:numel(zplaneHeight)
        ...
        hold on
        %transmitters.drawpattern('followPos')
        walls.drawself('excludeGround','color',min(LosRssi_part(:)));
        transmitters.drawpos
...
if refFlag==1%first reflection
    ...
    for idx=1:numel(zplaneHeight)
        ...
        hold on
        %transmitters.drawpattern('followPos')
        walls.drawself('excludeGround','color',min(RefRssi_part(:)));
        transmitters.drawpos
...
if secRefFlag==1%second reflection
    ...
    for idx=1:numel(zplaneHeight)
        ...
        hold on
        %transmitters.drawpattern('followPos')
        walls.drawself('excludeGround','color',min(secRefRssi_part(:)));
        transmitters.drawpos
...
for idx=1:numel(zplaneHeight)
    ...
    hold on
    %transmitters.drawpattern('followPos')
    walls.drawself('excludeGround','color',min(TotalRssi_part(:)));
    transmitters.drawpos
...
figure()
plot(Rx.xyz(idx,1),Rx.TotalRssi_dB(idx));
xlabel('X - Metre')
