...
%                                       1/(Tx2Refdist+Rx2Refdist) * prod(tempFresnelCoeff_forward)*prod(tempFresnelCoeff_back)*RefCoeff*exp(1j*(-transmitters.members(a).k*(Tx2Refdist+Rx2Refdist)+transmitters.members(a).phase));
                        Rx.RefRssi.x(b)=Rx.RefRssi.x(b)+...
                                      sqrt( transmitters.members(a).power*TxGain.x*RxGain.x*...%exp(1j*TxGain.Ex_ang*2)*exp(-1j*RxGain.Ex_ang*2)*...
                                      (transmitters.members(a).lambda/(4*pi))^2 ) *...
                                      1/(Tx2Refdist+Rx2Refdist) * prod(tempFresnelCoeff_forward.x)*prod(tempFresnelCoeff_back.x)*RefCoeff.x*exp(1j*(-transmitters.members(a).k*(Tx2Refdist+Rx2Refdist)+transmitters.members(a).phase))... %additional terms by adding 3polarization
                                       ;%*TxGain.Ex_amp*exp(1j*TxGain.Ex_ang)*RxGain.Ex_amp*exp(-1j*RxGain.Ex_ang);
...
%                                               1/(d1+d2+d3) * prod(tempFresnelCoeff_path1)*RefCoeff_p1*prod(tempFresnelCoeff_path2)*RefCoeff_p2*prod(tempFresnelCoeff_path3)*exp(1j*(-transmitters.members(a).k*(d1+d2+d3)+transmitters.members(a).phase));
                                Rx.secRefRssi.x(b)=Rx.secRefRssi.x(b)+...
                                              sqrt( transmitters.members(a).power*TxGain.x*RxGain.x*exp(1j*TxGain.Ex_ang*2)*exp(-1j*RxGain.Ex_ang*2)*...
                                              (transmitters.members(a).lambda/(4*pi))^2 ) *...
                                              1/(d1+d2+d3) * prod(tempFresnelCoeff_path1.x)*RefCoeff_p1.x*prod(tempFresnelCoeff_path2.x)*RefCoeff_p2.x*prod(tempFresnelCoeff_path3.x)*exp(1j*(-transmitters.members(a).k*(d1+d2+d3)+transmitters.members(a).phase))...%additional terms by 3 polarization
                                              ;%*TxGain.Ex_amp*exp(1j*TxGain.Ex_ang)*RxGain.Ex_amp*exp(-1j*RxGain.Ex_ang);
