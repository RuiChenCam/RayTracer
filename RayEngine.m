function [LosRssi,RefRssi,secRefRssi] = RayEngine(losFlag,refFlag,secRefFlag,polarizationSwap,RxPos)
  global transmitters walls receivers;
...
%                               1/RxTx.dist(b,:,a) * prod(tempFresnelCoeff)*exp(1j*(-transmitters.members(a).k*RxTx.dist(b,:,a)+transmitters.members(a).phase));
                ...
                Rx.LosRssi.x(b)=Rx.LosRssi.x(b)+...
                              sqrt( transmitters.members(a).power*transmitters.members(a).gain(losBeamAngle.Tx.Zen(b,:,a),losBeamAngle.Tx.Azi(b,:,a)).x*RxGain.x*...
                              (transmitters.members(a).lambda/(4*pi))^2 ) *...
                              1/RxTx.dist(b,:,a) * sqrt(prod(tempFresnelCoeff.x))*exp(1j*(-transmitters.members(a).k*RxTx.dist(b,:,a)+transmitters.members(a).phase));
                          
                Rx.LosRssi.y(b)=Rx.LosRssi.y(b)+...
                              sqrt( transmitters.members(a).power*transmitters.members(a).gain(losBeamAngle.Tx.Zen(b,:,a),losBeamAngle.Tx.Azi(b,:,a)).y*RxGain.y*...
                              (transmitters.members(a).lambda/(4*pi))^2 ) *...
                              1/RxTx.dist(b,:,a) * sqrt(prod(tempFresnelCoeff.y))*exp(1j*(-transmitters.members(a).k*RxTx.dist(b,:,a)+transmitters.members(a).phase));
                          
                Rx.LosRssi.z(b)=Rx.LosRssi.z(b)+...
                              sqrt( transmitters.members(a).power*transmitters.members(a).gain(losBeamAngle.Tx.Zen(b,:,a),losBeamAngle.Tx.Azi(b,:,a)).z*RxGain.z*...
                              (transmitters.members(a).lambda/(4*pi))^2 ) *...
...
%                                       1/(Tx2Refdist+Rx2Refdist) * prod(tempFresnelCoeff_forward)*prod(tempFresnelCoeff_back)*RefCoeff*exp(1j*(-transmitters.members(a).k*(Tx2Refdist+Rx2Refdist)+transmitters.members(a).phase));
                        Rx.RefRssi.x(b)=Rx.RefRssi.x(b)+...
                                      sqrt( transmitters.members(a).power*transmitters.members(a).gain(RefBeamAngle.Tx.Zen,RefBeamAngle.Tx.Azi).x*RxGain.x*...
                                      (transmitters.members(a).lambda/(4*pi))^2 ) *...
                                      1/(Tx2Refdist+Rx2Refdist) * sqrt(prod(tempFresnelCoeff_forward.x)*prod(tempFresnelCoeff_back.x)*RefCoeff.x)*exp(1j*(-transmitters.members(a).k*(Tx2Refdist+Rx2Refdist)+transmitters.members(a).phase));
                        Rx.RefRssi.y(b)=Rx.RefRssi.y(b)+...
                                      sqrt( transmitters.members(a).power*transmitters.members(a).gain(RefBeamAngle.Tx.Zen,RefBeamAngle.Tx.Azi).y*RxGain.y*...
                                      (transmitters.members(a).lambda/(4*pi))^2 ) *...
                                      1/(Tx2Refdist+Rx2Refdist) * sqrt(prod(tempFresnelCoeff_forward.y)*prod(tempFresnelCoeff_back.y)*RefCoeff.y)*exp(1j*(-transmitters.members(a).k*(Tx2Refdist+Rx2Refdist)+transmitters.members(a).phase));
                        Rx.RefRssi.z(b)=Rx.RefRssi.z(b)+...
                                      sqrt( transmitters.members(a).power*transmitters.members(a).gain(RefBeamAngle.Tx.Zen,RefBeamAngle.Tx.Azi).z*RxGain.z*...
                                      (transmitters.members(a).lambda/(4*pi))^2 ) *...
...
%                                               1/(d1+d2+d3) * prod(tempFresnelCoeff_path1)*RefCoeff_p1*prod(tempFresnelCoeff_path2)*RefCoeff_p2*prod(tempFresnelCoeff_path3)*exp(1j*(-transmitters.members(a).k*(d1+d2+d3)+transmitters.members(a).phase));
                                Rx.secRefRssi.x(b)=Rx.secRefRssi.x(b)+...
                                              ...
                                              (transmitters.members(a).lambda/(4*pi))^2 ) *...
                                              1/(d1+d2+d3) * sqrt(prod(tempFresnelCoeff_path1.x)*RefCoeff_p1.x*prod(tempFresnelCoeff_path2.x)*RefCoeff_p2.x*prod(tempFresnelCoeff_path3.x))*exp(1j*(-transmitters.members(a).k*(d1+d2+d3)+transmitters.members(a).phase));
                                Rx.secRefRssi.y(b)=Rx.secRefRssi.y(b)+...
                                              sqrt( transmitters.members(a).power*TxGain.y*RxGain.y*...
                                              (transmitters.members(a).lambda/(4*pi))^2 ) *...
                                              1/(d1+d2+d3) * sqrt(prod(tempFresnelCoeff_path1.y)*RefCoeff_p1.y*prod(tempFresnelCoeff_path2.y)*RefCoeff_p2.y*prod(tempFresnelCoeff_path3.y))*exp(1j*(-transmitters.members(a).k*(d1+d2+d3)+transmitters.members(a).phase));
